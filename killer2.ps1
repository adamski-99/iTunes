...
$FrmItunes.controls.AddRange(@($ButtonCancel,$ButtonOk,$ComboPlaylists,$Label,$Label1,$TextBox1,$cbCompilation,$dgvResults,$lbProgess,$txtCurrent,$lbOf,$txtTotal,$lbtracks))
...
function fnChangeAlbumArtists { }
        ...
        $txtTotal.text = $iTunes.LibrarySource.Playlists.ItemByName($ComboPlaylists.SelectedItem).count

        foreach ( $track in $iTunes.LibrarySource.Playlists.ItemByName($ComboPlaylists.SelectedItem).Tracks) {
